...
public function get_post_list($page=1,$type_id=0,$status=0,$uid=0,$key="",$type=0){
	...
	$row=$this->M->get_one("SELECT count(*) as a from `lx_post` where $str");
	$page2=$this->Page($page,$row['a'],25);	 
	$select_from=$page2['select_from'];
	$select_limit=$page2['select_limit'];	 
	$arr=$this->M->get_all("SELECT * from `lx_post` where $str order by $order limit $select_from,$select_limit");
	...
	foreach ($arr as $key => &$e) {
		...
		$e['item_type']=$this->get_type_name($e['type_id']);
		$e['summary']=htmlspecialchars_decode($e['summary']);


		$e['color']=$color[array_rand($color,1)];
			...
			if ($fnm['is_edit']==1) {
				...
				switch ($fnm['type']) {
					...
					case '2':
						...
						$new_arr[$i][0]='<textarea cols="50" rows="4" style="height:100px" class="form-control1" name="'.$e[1].'">'.$e[2].'</textarea>';
						break;	
					case '3':
...
public function download_add(){
	$this->display("download_add.html");
}
public function save_file(){
	$data=$_POST;
	$data['c_time']=time();
	$data['status'];
	$this->M->insert("lx_file",$data);
