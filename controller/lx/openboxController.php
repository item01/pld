...
/*红包参数设置*/
	...
	public function box_set(){

		$arr=$this->M->get_all("SELECT * from `lx_openbox_box`");	
		$arr3=$this->M->get_one("SELECT * from `lx_openbox_set`");	
		$arr2=$this->M->get_all("SELECT * from `lx_base_prize` where `status`=1");	
		if ($arr) {
			foreach ($arr as $key => &$e) {
				$arr_f=$this->M->get_one("SELECT * from `lx_base_prize` where `id`='".$e['pid']."'");	
				$e['p_name']=$arr_f['prizename'];
		...
		$this->display("box_set.html");
	}




	public function openbox_do_set(){
		...
		$this->M->update('lx_openbox_set',$data,"`id`=1");

		
		$id=$this->M->affected_rows();
		echo 1;
		//message("修改成功",R('lx/openbox/openbox_set'));exit;
...
/**/
	...
	public function box_edit($id=''){
		...
		$arr=$this->M->get_one("SELECT * from `lx_openbox_box` where `id`='".$id."'");
		
		$arr3=$this->M->get_all("SELECT * from `lx_base_prize` where `status`=1");
		$this->assign("list2",$arr);
		$this->assign("list3",$arr3);

		$this->display("box_edit.html");
	}

	public function box_do_add(){
		...
		$data['status']=1;
		$this->M->insert('lx_openbox_box',$data);		
		$id=$this->M->insert_id();
	...
	public function box_do_edit($id=''){
		...
		$this->M->update('lx_openbox_box',$data,"`id`='".$id."'");
	...
	public function box_do_delete(){
		$id=$_POST['id'];
		$this->M->query("DELETE from `lx_openbox_box` where `id`='".$id."'");
...
public function yaohui_do_set(){
	...
	public function gift_set(){
		...
		$arr=$this->M->get_all("SELECT * from `lx_kanjia_gift`");	
		$this->assign('list',$arr);
