...
class DBManage {
     ...
     */
    function __construct($host = 'localhost', $username = 'root', $password = 'root', $database = 'lx_zc', $charset = 'utf8mb4') {
        $this->host = $host;
...
//创建目录
        $dir2=$dir.$filename.'/';
        if (! is_dir ( $dir2 )) {
            mkdir ( $dir2, 0777, true ) or die ( '创建文件夹失败' );
        }
            ...
            while ( $record = mysql_fetch_array ( $data ) ) {
                ...
                if (strlen ( $sql ) >= $size * 1024) {
                    ...
                    if ($this->_write_file ( $sql, $file, $dir2 )) {
            ...
            //if (!file_exists($dir.'/'.$filename.'/')){ mkdir ($dir.'/'.$filename.'/'); 

            // sql大小不够分卷大小
            if ($sql != "") {
                $filename .= "_v" . $p . ".sql";
                if ($this->_write_file ( $sql, $filename, $dir2 )) {
